...
class AnticipatoryPlanner:
    ...
    def __init__(
        self,
        simulator: PredictiveSimulationEngine,
        runtime: TaskGraphRuntime,
        governance: CognitiveGovernanceEngine | None = None,
        watchdog: CognitiveWatchdog | None = None,
        tracer: DecisionTraceLedger | None = None,
    ) -> None:
        self.simulator = simulator
        self.runtime = runtime
        self.governance = governance or CognitiveGovernanceEngine()
        self.watchdog = watchdog or CognitiveWatchdog()
        self.tracer = tracer or DecisionTraceLedger()

    async def plan_and_execute(
        self, goal: Goal, strategies: list[list[TaskNode]], tier: int = 1
    ) -> bool:
        ...
        if profile.get("force_heuristic"):
            tier = 0
            logger.warning(
                "Cognitive pressure high (%.2f). Downgrading to Tier 0 heuristic simulation.",
                pressure,
            )
        ...
        if tier > 0 and not self.governance.consume_llm_call():
            logger.error(
                "LLM budget exceeded. Cannot perform deep simulation for goal %s.", goal.name
            )
            return False
        ...
        scored_scenarios.sort(
            key=lambda s: self.simulator.risk_engine.evaluate_scenario(s), reverse=True
        )
