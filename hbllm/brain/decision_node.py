            ...
            try:
                ...
                if not result.passed:
                    ...
                    err_msg = Message(
                        ...
                        payload={
                            "text": f"I cannot fulfill this request — policy violation: {violations}"
                        },
    ...
    async def _route_to_execution(
        ...
        if intent == "speak" or original_query.get("force_audio", False):
            ...
            await self.bus.publish(
                "sensory.audio.out", _make_msg("sensory.audio.out", {"text": content})
            )
        ...
        # Python code execution
            ...
            await self.bus.publish(
                "task.execute.python",
                _make_msg("task.execute.python", {"code": code}, MessageType.QUERY),
            )
        ...
        # Default: text output to user
            ...
            await self.bus.publish(
                "sensory.output",
                _make_msg("sensory.output", {"text": content, "source": thought_type}),
            )
