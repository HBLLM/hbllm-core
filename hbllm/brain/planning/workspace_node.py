...
class WorkspaceNode(Node, IWorkspace):
    ...
    @property
    def thinking_deadline(self) -> float:
        return self._thinking_deadline
    ...
    async def handle_update(self, message: Message) -> Message | None:
        ...
        if len(self.blackboards) >= self._max_concurrent_boards:
            oldest_ids = sorted(
                self.blackboards,
                key=lambda cid: float(self.blackboards[cid].get("start_time", 0)),
            )[:10]
        ...
        from hbllm.brain.core.factory import _is_slow_cpu

        is_slow = _is_slow_cpu()

        is_fast_path = payload.get("is_fast_path", False)
        # All queries resolve via early consensus as soon as the expected thought arrives.
        # Allow the full thinking deadline (default 60s) so cloud and local generation can complete.
        thinking_time = self._thinking_deadline
        # Scale absolute deadline: 300s for complex CPU path, 120s for GPU/Cloud path
        abs_deadline = 300.0 if is_slow else 120.0
    ...
    async def handle_thought(self, message: Message) -> Message | None:
        ...
        if thought_type == "symbolic_logic" and confidence == 1.0:
            is_intermediate = bool(proposal.get("is_intermediate", False))
            if is_intermediate:
                logger.info(
                ...
                new_payload = dict(board["original_query"]).copy()
                new_payload["text"] = (
    ...
    async def _consensus_watcher(self, corr_id: str) -> None:
    ...
    async def _finalize_board(self, corr_id: str) -> None:
        """
        ...
        """
        board = self.blackboards.get(corr_id)
        if not board or bool(board["resolved"]):
            return
        ...
        content = str(best_thought.get("content", ""))
    ...
    async def _inject_memory_context(self, corr_id: str, board: dict[str, Any]) -> None:
        ...
        async def _try_semantic() -> None:
            ...
            try:
                ...
                results = list(resp.payload.get("results", []))
                if results:
                    memory_context = "\n".join(
        ...
        async def _try_procedural() -> None:
            ...
            try:
                ...
                skills = list(resp.payload.get("skills", []))
                if skills:
                    skill_text = "\n".join(
    ...
    async def _commit_to_decision(self, corr_id: str, best_thought: dict[str, Any]) -> None:
        ...
        while True:
            try:
                ...
                stale_ids = [
                    ...
                    if now - float(board.get("start_time", now)) > self._max_board_age
                ]
