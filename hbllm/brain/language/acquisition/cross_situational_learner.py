...
class CrossSituationalLearner:
    ...
    def observe_situated_token(
        ...
        spatial_edges: list[tuple[str, str, str]]
        | None = None,  # (source_id, edge_type, target_id)
        ...
        for entity_id in visible_entity_ids:
            ...
            for prop_key, prop_val in props.items():
                if prop_key in ("freshness", "timestamp", "location", "shape") or not isinstance(
                    prop_val, (str, int, float)
                ):
                    continue
        ...
        if state_delta:
            action_name = str(
                state_delta.get("action", state_delta.get("transition", "transition"))
            )
        ...
        for candidate in hypothesis_set.candidates:
            if (
                candidate.target_type == LexicalTargetType.INDIVIDUAL
                and candidate.target_id not in visible_entity_ids
            ):
