...
def run_plugin_new(args):
    ...
    init_content = f'''"""
{args.name} HBLLM Plugin.
Generated by hbllm CLI.
"""
...
class {plugin_name.title().replace("_", "")}Plugin(HBLLMPlugin):
    """My custom plugin logic."""

    @subscribe("example.topic")
    async def handle_example(self, message: Message) -> None:
        print(f"[{plugin_name}] Received message: {{message.payload}}")
'''
    (plugin_dir / "__init__.py").write_text(init_content)
    logging.info("Successfully scaffolded plugin '%s' at %s", args.name, plugin_dir)
...
def run_plugin(args):
    ...
    if args.plugin_cmd == "list":
        ...
        print("Usage: hbllm plugin [list|new]")
...
def main():
    ...
    plugin_parser = subparsers.add_parser("plugin", help="Manage plugins (list, new)")
    plugin_sub = plugin_parser.add_subparsers(dest="plugin_cmd", required=True)
    plugin_sub.add_parser("list", help="List all installed dynamic plugins")
    ...
    # Lazy imports to avoid pulling in heavy deps for non-agent commands
    def _run_agent(a):
    ...
    def _run_code(a):
        from hbllm.cli.agent import run_code
