"""
Stochastic Robustness Multi-Tier Benchmark Suite.

Evaluates Pure HCIR epistemic belief maintenance and surprise recovery across
actuator slip, sensory dropout, and dynamic drift tiers.
"""
...
from .environment import make_stochastic_env
...
class PureHCIRStochasticAgent:
    """Surprise-reactive agent maintaining epistemic beliefs under noise."""

    def __init__(self) -> None:
        self.perception = StochasticPerceptionAdapter()
        self.action = StochasticActionAdapter()
    ...
    def select_action(self, obs: StochasticObservation) -> StochasticAction:
        """Process observation and yield closed-loop action."""
        percept = self.perception.process_observation(obs)
        return self.action.select_action(obs, self.perception, percept)


def run_stochastic_tier(
    tier: StochasticTier | str,
    episodes: int = 5,
    seed: int = 42,
) -> dict[str, Any]:
    """Run evaluation on a single stochastic robustness tier."""
    agent = PureHCIRStochasticAgent()
    tier_enum = StochasticTier(tier)
    ...
    for ep in range(episodes):
        agent.reset_episode()
        env = make_stochastic_env(tier=tier_enum, seed=seed + ep)
        obs = env.reset()
        ...
        while not done:
            ...
            if info.get("was_slipped", False):
                total_slips += 1
            if info.get("was_occluded", False):
                total_occlusions += 1
...
def run_stochastic_benchmark(
    episodes_per_tier: int = 5,
    seed: int = 42,
    episodes: int | None = None,
) -> dict[str, Any]:
    """Run full 5-tier stochastic robustness battery."""
    if episodes is not None:
        episodes_per_tier = max(1, episodes // len(StochasticTier))
    ...
    for tier in StochasticTier:
        res = run_stochastic_tier(tier, episodes=episodes_per_tier, seed=seed)
    ...
    return {
        "benchmark": "stochastic_robustness",
        "tiers": tiers_results,
