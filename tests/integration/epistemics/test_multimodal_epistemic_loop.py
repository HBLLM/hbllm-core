...
class TestMultimodalEpistemicLoop:
        ...
        aud_assessment = AudioAssessment(
            ...
            events=[
                SoundEventEvidence(
                    event_type="speech",
                    confidence=0.95,
                    provider_provenance=ProviderProvenance(provider="yamnet", model="v1", version="1.0", device="cpu"),
                )
            ],
    ...
    async def test_cross_modal_contradiction_investigation(
        ...
        contradictions = [
            n for _n in graph.all_nodes()
            if isinstance((n := graph.get_node(_n.id)), ContradictionNode)
        ]
        assert len(contradictions) >= 1
        l2_contra = contradictions[0]
        assert l2_contra.contradiction_level == PerceptualContradictionLevel.LEVEL_2_CROSS_MODAL_CONFLICT
