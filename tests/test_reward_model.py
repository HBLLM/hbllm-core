...
class TestRewardSignalRecording:
    def test_record_explicit_reward(self, reward_model):
        signal = RewardSignal(
            query="What is Python?", response="A programming language.", reward=0.9
        )
...
class TestScoring:
    ...
    def test_very_short_response_penalized(self, reward_model):
        ...
        normal = reward_model.score_response(
            "Explain AI",
...
class TestExport:
    def test_export_preferences(self, reward_model):
        for i in range(3):
            reward_model.record_preference(
                PreferencePair(
                    query=f"q{i}",
                    chosen=f"good{i}",
                    rejected=f"bad{i}",
