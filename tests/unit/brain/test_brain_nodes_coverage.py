...
def _mock_llm():
    ...
    llm.generate = AsyncMock(return_value=MagicMock(
        text="Test response", content="Test response",
...
def _mock_bus():
    ...
    bus.request = AsyncMock(return_value=MagicMock(
...
class TestDecisionNode:
    ...
    def test_init_with_llm(self, tmp_path):
        from hbllm.brain.decision_node import DecisionNode
        node = DecisionNode(
            node_id="decision_test", llm=_mock_llm(), data_dir=str(tmp_path)
        )
        assert node is not None
...
class TestPlannerNode:
    ...
    def test_init_with_llm(self):
        from hbllm.brain.planner_node import PlannerNode
        node = PlannerNode(
            node_id="planner_test", llm=_mock_llm(),
            branch_factor=2, max_depth=1
        )
        assert node is not None
...
class TestSleepCycleNode:
    ...
    def test_node_type(self):
        from hbllm.brain.sleep_node import SleepCycleNode, SleepPhase
        node = SleepCycleNode(node_id="sleep_test")
        info = node.get_info()
