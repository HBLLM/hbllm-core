...
mod tests {
    ...
    fn test_deny_block_matching() {
        let policies = vec![make_policy("no_secrets", "deny", "block", r"password\s*=", "", "critical")];
        let ctx = HashMap::new();
    ...
    fn test_deny_replace() {
        let policies = vec![make_policy("redact_email", "deny", "replace", r"\S+@\S+\.\S+", "[EMAIL]", "medium")];
        let ctx = HashMap::new();
        let result = evaluate_policies("Contact me at user@example.com please", &policies, "t1", "", &ctx);
        assert!(result.passed);
    ...
    fn test_require_missing_blocks() {
        let policies = vec![make_policy("require_disclaimer", "require", "block", r"DISCLAIMER", "", "high")];
        let ctx = HashMap::new();
        let result = evaluate_policies("Here is my response to your question", &policies, "t1", "", &ctx);
        assert!(!result.passed);
    }
    ...
    fn test_require_present_passes() {
        let policies = vec![make_policy("require_disclaimer", "require", "block", r"DISCLAIMER", "", "high")];
        let ctx = HashMap::new();
        let result = evaluate_policies("DISCLAIMER: This is informational only.", &policies, "t1", "", &ctx);
        assert!(result.passed);
    }

    #[test]
    fn test_require_append() {
        let policies = vec![make_policy("add_footer", "require", "append", r"FOOTER", "\n---\nGenerated by HBLLM", "low")];
        let ctx = HashMap::new();
    ...
    fn test_transform_prepend() {
        let policies = vec![make_policy("add_header", "transform", "prepend", "", "## Header", "low")];
        let ctx = HashMap::new();
    ...
    fn test_scope_restrict_allowed() {
        let mut policy = make_policy("scope_math", "scope", "restrict", "", "math,science", "high");
    ...
    fn test_scope_restrict_denied() {
        let policy = make_policy("scope_math", "scope", "restrict", "", "math,science", "high");
        let ctx = HashMap::new();
    ...
    fn test_condition_numeric_gte() {
        ...
        policy.conditions = vec![("risk_score".to_string(), "gte".to_string(), "0.8".to_string())];
